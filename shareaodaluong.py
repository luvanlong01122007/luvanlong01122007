...
def main(post_id, token_dict, total_shares, chống_block1, chống_block, da_luong):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=da_luong) as executor:
        for start in range(0, total_shares, chống_block1):
            ...
            for share_number in range(start + 1, end + 1):
                ...
                futures.append(executor.submit(share_post, token, post_id, share_number))
            concurrent.futures.wait(futures)
            
            if start + chống_block1 < total_shares:
                cl()
                print(f"{fg('green')}Chống Block Menu {attr('reset')}")
...
if __name__ == "__main__":
    ...
    ngta_đặt_thanh()
    try:
        main(post_id, token_dict, total_shares, chống_block1, chống_block, da_luong)
    except Exception as e:
        ngta_đặt_thanh()
